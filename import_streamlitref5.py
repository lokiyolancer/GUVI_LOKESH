...
queries = {
    ...
    :"""SELECT YEAR(orderfirst.order_date) AS year,SUM(orderlast.sale_price) AS total_revenue
      FROM orderfirst
      JOIN orderlast ON orderfirst.order_id = orderlast.order_id
...
left_queries = query_keys[:7]   # First 5 queries for left column
right_queries = query_keys[7:]  # Next 5 queries for right column

# ✅ Two Columns for Query Selection
col1,spacer, col2 = st.columns([2,0.5, 2])

selected_query = None  # Stores selected query



# ✅ Left Column (First 5 Queries)
with col1:
    st.subheader(" Left Side Queries")
    for query_name in left_queries:
        if st.button(query_name):  # If button is clicked, store query
            selected_query = queries[query_name]

# ✅ Right Column (Next 5 Queries)
with col2:
    st.subheader(" Right Side Queries")
    for query_name in right_queries:
        if st.button(query_name):  # If button is clicked, store query
            selected_query = queries[query_name]

# ✅ If a Query is Selected, Execute it
if selected_query:
